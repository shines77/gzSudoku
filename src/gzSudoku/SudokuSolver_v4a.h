...
private:
    ...
    LiteralInfoEx count_literal_size_init_v1() {
        ...
        for (box = 0; box < Boxes - 1; box += 2) {
            ...
            disable_mask1.and_equal(numbits_mask);
            box_bits_1.or_equal(disable_mask1);

            BitVec16x16_AVX disable_mask2 = box_bits_2.whichIsZeros();
            disable_mask2.and_equal(numbits_mask);
            box_bits_2.or_equal(disable_mask2);
...
public:
    ...
    int solve(const Board & board, Board & solution, int limitSolutions = 1) {
        ...
        assert((ptrdiff_t)empties <= (Sudoku::BoardSize - Sudoku::MinInitCandidates));
